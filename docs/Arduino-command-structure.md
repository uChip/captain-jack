...
To have realistic (lifelike) movement the pitch (head nod up and down), roll (head tips side to side), and yaw (head rotates side to side) have to be synchronized and eased.  An easing library is available on the Arduino that overlays the servo library.  
 ...
 - **Superseded 2026-09-15**: the fixed-width syntax above (2/2/3/4-char zero-padded fields) is no longer the wire format. The Arduino sketch actually written (`arduino/ServoControl/ServoControl.ino`) uses variable-length, self-delimiting integers instead - each field is a command char followed by a decimal integer of any length, terminated by whatever non-numeric character comes next, no zero-padding. This is now locked down as the authoritative syntax - see docs/specification.md section 4.13. The char-count/timing math on the line above is moot: it was computed for the fixed-width format and doesn't apply to the variable-length one, and per the "Open questions" resolution below, that math isn't needed anyway.
 - Open questions:
   ...
   - ~~How robust is serial communications? Is an ACK or timeout retry needed?~~ Resolved 2026-09-15, Chip's call: no ACK/timeout-retry needed - the variable-length syntax's self-resyncing design (any unrecognized byte is simply discarded, with no positional/framing state to lose sync) is robust enough on its own. See docs/specification.md, Open Issues issue 9.
